...
BEGIN
  ...
  SELECT 
    ...
    (SELECT ano FROM reajustes_anuais WHERE percentual = (SELECT MAX(percentual) FROM reajustes_anuais) LIMIT 1) as ano_maior_reajuste,
    MIN(percentual) as menor_reajuste,
    (SELECT ano FROM reajustes_anuais WHERE percentual = (SELECT MIN(percentual) FROM reajustes_anuais) LIMIT 1) as ano_menor_reajuste
