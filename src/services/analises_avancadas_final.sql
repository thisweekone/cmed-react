...
-- Primeiro removemos as funções existentes para evitar conflitos de tipos de retorno
DROP FUNCTION IF EXISTS obter_anos_disponiveis();
DROP FUNCTION IF EXISTS pg_get_function_def(TEXT, TEXT);
...
DROP FUNCTION IF EXISTS analise_elasticidade_por_faixa(INT); -- Função com apenas um parâmetro
DROP FUNCTION IF EXISTS projecao_impacto_financeiro(INT, NUMERIC);
DROP FUNCTION IF EXISTS obter_projecoes_reajuste(INT);
DROP FUNCTION IF EXISTS verificar_estrutura_tabela(TEXT, TEXT);

-- Função para verificar a estrutura da tabela
CREATE OR REPLACE FUNCTION verificar_estrutura_tabela(
  p_schema TEXT,
  p_tabela TEXT
)
RETURNS TABLE (
...
BEGIN
  ...
  WITH medicamentos_historico AS (
    SELECT 
      ...
      m.apresentacao,
      m.laboratorio,
      m.tipo_produto AS tipo, -- Alterado de tipo_de_produto para tipo_produto
...
BEGIN
  ...
  WITH medicamentos_agregados AS (
    SELECT 
      ...
      m.tipo_produto, -- Alterado de tipo_de_produto para tipo_produto
    ...
    WHERE 
      ...
      AND m.tipo_produto IN ('Genérico', 'Referência') -- Alterado de tipo_de_produto para tipo_produto
    GROUP BY 
      ano_med, m.tipo_produto
    HAVING 
...
BEGIN
  ...
  IF desvio_padrao IS NULL OR desvio_padrao = 0 THEN
    ...
    IF desvio_padrao = 0 THEN
      desvio_padrao := 1; -- Evita divisão por zero
    END IF;
  END IF;
  
  -- Calcular tamanho de cada faixa
  faixa_tamanho := (preco_max - preco_min) / GREATEST(faixas_preco, 1);
  IF faixa_tamanho = 0 THEN 
    faixa_tamanho := 1; -- Evita divisão por zero
  END IF;
    ...
    SELECT 
      ...
      CASE 
        ...
        ELSE -- Uso de faixas numéricas para outros casos
          'Faixa ' || FLOOR((ph.preco_maximo_consumidor - preco_min) / GREATEST(faixa_tamanho, 0.01)) + 1
      END AS faixa_preco,
      -- Min e Max da faixa
      CASE 
        WHEN FLOOR((ph.preco_maximo_consumidor - preco_min) / GREATEST(faixa_tamanho, 0.01)) = 0 THEN preco_min
        ELSE preco_min + (FLOOR((ph.preco_maximo_consumidor - preco_min) / GREATEST(faixa_tamanho, 0.01)) * faixa_tamanho)
      END AS faixa_min,
      CASE 
        WHEN FLOOR((ph.preco_maximo_consumidor - preco_min) / GREATEST(faixa_tamanho, 0.01)) >= faixas_preco - 1 THEN preco_max
        ELSE preco_min + ((FLOOR((ph.preco_maximo_consumidor - preco_min) / GREATEST(faixa_tamanho, 0.01)) + 1) * faixa_tamanho)
      END AS faixa_max,
