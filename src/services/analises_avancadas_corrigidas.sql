...
-- Função para obter a definição de uma função
CREATE OR REPLACE FUNCTION pg_get_function_def(function_name TEXT)
RETURNS TEXT AS $$
DECLARE
  func_def TEXT;
BEGIN
...
BEGIN
  ...
  RETURN QUERY
  SELECT DISTINCT EXTRACT(YEAR FROM data_publicacao)::INT AS ano_valor
...
BEGIN
  ...
  WHERE 
    mcv.variacao_percentual BETWEEN -50 AND 50 -- Filtrar outliers extremos
  GROUP BY 
    mcv.ano, mcv.tipo_de_produto, ra.percentual_reajuste
  ORDER BY 
    mcv.ano, mcv.tipo_de_produto;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  -- Criar tabela para armazenar resultados temporários
  CREATE TEMP TABLE IF NOT EXISTS temp_precos_historicos (
    ano INT,
    preco_medio NUMERIC,
    variacao_percentual NUMERIC,
    impacto_acumulado NUMERIC,
  ...
  -- Limpar tabela temporária se já existir
