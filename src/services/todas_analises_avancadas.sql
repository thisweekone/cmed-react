-- ========================================================================
-- FUNÇÕES PARA ANÁLISES AVANÇADAS DE PREÇOS DE MEDICAMENTOS
-- ========================================================================
...
BEGIN
  ...
  WITH medicamentos_com_variacao AS (
    -- Obter variação percentual por ano
    SELECT 
      EXTRACT(YEAR FROM ph.data_publicacao)::INT AS ano,
      m.id,
      m.produto,
  ...
  medias_anuais AS (
    ...
    WHERE 
      variacao_percentual BETWEEN -50 AND 50 -- Filtrar outliers extremos
    GROUP BY 
      ano
  )
  SELECT 
...
BEGIN
  ...
  reajuste_do_ano AS (
    -- Obter o reajuste CMED para o ano de referência
    SELECT percentual_reajuste
  ...
  SELECT 
    mcv.faixa_preco,
    AVG(mcv.variacao_percentual) AS variacao_media,
    (SELECT percentual_reajuste FROM reajuste_do_ano) AS reajuste_cmed,
    AVG(mcv.variacao_percentual) - (SELECT percentual_reajuste FROM reajuste_do_ano) AS diferenca,
    CASE 
      WHEN (SELECT percentual_reajuste FROM reajuste_do_ano) > 0 THEN
        AVG(mcv.variacao_percentual) / (SELECT percentual_reajuste FROM reajuste_do_ano)
      ELSE 1.0
    END AS elasticidade,
    COUNT(DISTINCT mcv.id) AS total_medicamentos
  FROM 
...
-- ========================================================================
-- 4. ANÁLISE DE GENÉRICOS VS. REFERÊNCIA
-- ========================================================================
...
BEGIN
  ...
  estatisticas AS (
    ...
    SELECT 
      AVG(diferenca) AS media,
      STDDEV(diferenca) AS desvio_padrao
    FROM 
      medicamentos_com_variacao
    WHERE 
      variacao_percentual BETWEEN -50 AND 50 -- Filtrar outliers extremos iniciais
  )
  -- Criar tabela temporária com resultados
  CREATE TEMP TABLE temp_outliers AS
  SELECT 
    mcv.id,
    mcv.produto,
    mcv.apresentacao,
    ...
    mcv.diferenca,
    CASE
      WHEN mcv.diferenca > (SELECT media + 2 * desvio_padrao FROM estatisticas) THEN 'positivo'
      WHEN mcv.diferenca < (SELECT media - 2 * desvio_padrao FROM estatisticas) THEN 'negativo'
      ELSE 'normal'
    END AS tipo_outlier
  FROM 
    medicamentos_com_variacao mcv;
  
  -- Retornar resultados conforme o tipo solicitado
  RETURN QUERY
  SELECT * FROM temp_outliers
  WHERE (
    (tipo = 'ambos' AND tipo_outlier IN ('positivo', 'negativo', 'normal')) OR
    (tipo = 'positivo' AND tipo_outlier IN ('positivo', 'normal')) OR
    (tipo = 'negativo' AND tipo_outlier IN ('negativo', 'normal'))
  )
  ORDER BY 
    CASE
      WHEN tipo_outlier = 'positivo' THEN 1
      WHEN tipo_outlier = 'negativo' THEN 2
      ELSE 3
    END,
    ABS(diferenca) DESC;
...
BEGIN
  ...
  -- Calcular preço médio por ano (dados históricos)
  CREATE TEMP TABLE IF NOT EXISTS temp_precos_historicos AS
  ...
  SELECT AVG(variacao_percentual) INTO variacao_media
  FROM temp_precos_historicos
  ...
  SELECT preco_medio INTO preco_base
  FROM temp_precos_historicos
  WHERE ano = ano_fim;
  ...
  -- Retornar resultados
  RETURN QUERY
  SELECT * FROM temp_precos_historicos
  ORDER BY ano;
